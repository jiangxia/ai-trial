...
3. **角色激活清单**
   ...
   - 原告律师发言：prompx_action("lawyer") + 委托人身份设定 + **AI 宣誓** + **加载角色行为原则**
   - 被告一律师发言：prompx_action("lawyer") + 委托人身份设定 + **AI 宣誓** + **加载角色行为原则** + **强制执行被告一特殊规则**
   - 被告二律师发言：prompx_action("lawyer") + 委托人身份设定 + **AI 宣誓** + **加载角色行为原则** + **强制执行被告二特殊规则**
   - 被告三律师发言：prompx_action("lawyer") + 委托人身份设定 + **AI 宣誓** + **加载角色行为原则** + **强制执行被告三特殊规则**
...
- **智能评分**：系统对每份证据进行重要性评分（1-10 分）
- **分级质证**：根据评分决定质证深度，高分证据详细质证，低分证据快速确认

### 证据重要性评分标准

**10 分**：核心证据（合同原件、关键鉴定报告）
**7-9 分**：重要证据（银行流水、重要证人证言）
**4-6 分**：一般证据（普通函件、次要单据）
**1-3 分**：辅助证据（形式证据、补强证据）
...
4. **分级质证**：
   - **高分证据（≥7 分）**：详细质证，允许多轮交锋
   - **中分证据（4-6 分）**：简化质证，一轮确认
   - **低分证据（≤3 分）**：快速确认，无异议直接通过
...
详细流程请参考[争议焦点处理](./dispute_focus_integrated.md)
...
- **自动干预**：发言质量<4 分或连续 3 轮无新信息时自动触发法官干预
