...
2. **角色激活清单**
   ...
   - 原告律师发言：prompx_action("lawyer") + 委托人身份设定 + **AI宣誓**
   - 被告一律师发言：prompx_action("lawyer") + 委托人身份设定 + **AI宣誓**
   - 被告二律师发言：prompx_action("lawyer") + 委托人身份设定 + **AI宣誓**
   - 被告三律师发言：prompx_action("lawyer") + 委托人身份设定 + **AI宣誓**
   - 法官发言：prompx_action("judge")

   **⚠️ 重要：每次激活律师角色时必须默读AI宣誓，确保忠实代理委托人利益**
...
- **触发时机**：开庭前法官读取完所有材料后
- **执行动作**：分析起诉状 vs 答辞书，生成"庭前争议焦点.md"到案件目录
- **强制提醒**：向所有角色发送："请围绕以下争议焦点准备"

**开庭标准格式**："现在开庭。审理[案件性质]纠纷，庭前已识别争议焦点，请原告陈述诉讼请求和事实理由。"
...
2. **系统评分**：自动评估证据重要性（1-10分）
3. **被告质证**：各被告依次就每份原告证据的真实性、合法性、关联性发表质证意见
4. **分级质证**：
   - **高分证据（≥7分）**：详细质证，允许多轮交锋
   - **中分证据（4-6分）**：简化质证，一轮确认
   - **低分证据（≤3分）**：快速确认，无异议直接通过
...
- **⚠️ 律师质证重点：关注证据链完整性**
  ...
  - 质疑证据之间的逻辑关联
- 高分证据进行详细质证，低分证据简化确认
- 连续3份证据无争议时，法官可宣布"后续同类证据一并确认"
- 法官严格控制质证顺序，确保证据归属清晰

### 4. 争议焦点最终确认

**⚠️ 重要：质证结束后必须生成最终争议焦点！**

**最终争议焦点生成流程：**

1. **重新识别**：法官基于质证情况重新识别争议焦点
2. **智能评分**：系统自动对每个争议焦点进行重要性评分（1-10分）
3. **轮次分配**：根据评分自动分配各争议点的辩论轮次（高分多轮，低分少轮）
4. **生成文件**：在案件目录下生成"最终争议焦点.md"（含评分和轮次分配）
5. **差异分析**：法官分析两版本差异，调整庭审重点
6. **⚠️ 证据链验证（法官关键工作）**：自动检测证据链断点，必须提醒补充关键证据
7. **强制提醒**：向所有角色发送："后续发言必须围绕以下焦点，偏离将被制止！"

**差异分析模板：**

```
法官分析：庭前预判争议焦点为[X]，质证后发现实际争议焦点为[Y]。
差异原因：[简述]
庭审重点调整：[具体调整]
```
...
- **实时评分**：每次发言后自动评分（信息增量+焦点相关性+逻辑完整性）
- **自动干预**：发言质量<4分或连续3轮无新信息时自动触发法官干预
